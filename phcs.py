...
class phi():
    ...
    def __read_config(self):
        ...
        config.readfp(open(os.path.abspath(os.path.join(sys.path[0], 'settings.conf'))))
        return config
    ...
    def __process_command_line_arguments(self, config):
        ...
        parser.add_argument("-z""--postalcode", action = "store", dest = "postal_code", default = config.get("MAIN", "POSTALCODE"), help = "Postal Code used for location based services like weather")
        parser.add_argument("--soft-postalcode", action = "store_true", dest = "soft_postal_code", default = config.get("MAIN", "USE_SOFT_POSTALCODE"), help = "Try to determine the postal code programmatically.\nNote: you will need to have GEOIP  city data somewhere, and have to set --geo-data-file also")
        parser.add_argument("--geo-data-file", action = "store", dest = "geo_data_file", default = config.get("MAIN", "GEO_IP_DATA_FILE"), help = "GeoIP Data File Location")
        ...
        if self.soft_postal_code:
            try:
                import pygeoip
                gi = pygeoip.GeoIP(os.path.abspath(self.geo_data_file))
                ip = requests.get('http://automation.whatismyip.com/n09230945.asp').content
                record = gi.record_by_addr(ip)
                if 'postal_code' in record:
                    self.postal_code = record['postal_code']
            except ImportError:
                print "pygeoip not found defaulting to set postalcode " + self.postal_code + "\nTo use soft postal_code:\n\tpip install pygeoip"
    ...
    def __init__(self):
        ...
        #speak thread
        self.speak_thread = voiceThread(self)
        self.speak_thread.start()
        #all other threads
        self.taw_thread = timeAndWeatherThread(self)
        self.taw_thread.start()
        self.cal_thread = calendarThread(self)
...
class timeAndWeatherThread(threading.Thread):
    def __init__(self, phi):
        threading.Thread.__init__(self)
        self._stopevent = threading.Event()
        self.phi = phi
    ...
    def get_weather(self):
        url = self.base_url + "/conditions/forecast/astronomy/tide/q/" + self.phi.postal_code + ".json"
                ...
                for event in events['items']:
                    ...
                    if 'date' in event_start:
                        ...
                        if time.minute == 0:
                            ...
                            timestr = "%d %d %s" % (int(time.strftime("%I")),
                                           time.minute, ampm)
...
class skelotonThread(threading.Thread):
